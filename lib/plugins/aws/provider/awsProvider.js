...
class AwsProvider {
  ...
  getCallerIdSourceValue() {
    const values = this.getValues(this, [
      ['options', 'callerIdentity'],
      ['serverless', 'config', 'callerIdentity'],
      ['serverless', 'service', 'provider', 'callerIdentity'],
    ]);
    return this.firstValue(values);
  }

  getCallerIdArn() {
    const callerIdSrouceValue = this.getCallerIdSourceValue();
    return callerIdSrouceValue.value;
  }
  ...
  getAccountInfo() {
    const callerIdArn = this.getCallerIdArn();
    if (callerIdArn) {
      return new Promise((resolve) => {
        const splitArn = _.split(callerIdArn, ':');
        const partition = _.nth(splitArn, 1);
        const accountId = _.nth(splitArn, 4);
        resolve({
          accountId,
          partition,
          arn: callerIdArn
